...
export async function searchLikeUsers(input: string) {
    ...
    var users:{name:string, id:string}[] = await db.select({id: schema.user.id, name:schema.user.username}).from(schema.user).where(like(schema.user.username, `${input}%`))
...
export async function getUser(inputID: string) {
    ...
    var user:{name:string, id:string}[] = await db.select({id: schema.user.id, name:schema.user.username}).from(schema.user).where(eq(schema.user.id, inputID))
